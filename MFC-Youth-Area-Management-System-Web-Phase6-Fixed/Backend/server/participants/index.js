...
async function createParticipant(req, res) {
  ...
  if (!isAreaAdminRole(profile.role)) return sendJson(res, 403, { ok: false, error: 'Only Area-level servant access can register event participants.' });
  const areaId = requireArea(profile);
...
async function updateParticipant(req, res) {
  const { supabase, profile } = await requireAuthenticatedProfile(req);
  if (!isAreaAdminRole(profile.role)) return sendJson(res, 403, { ok: false, error: 'Only Area-level servant access can edit event participants.' });
  const areaId = requireArea(profile);
...
async function deleteParticipant(req, res) {
  const { supabase, profile } = await requireAuthenticatedProfile(req);
  if (!isAreaAdminRole(profile.role)) return sendJson(res, 403, { ok: false, error: 'Only Area-level servant access can delete event participants.' });
  const areaId = requireArea(profile);
