...
async function createEvent(req, res) {
  ...
  requireAreaAdmin(profile, 'Only Area-level servant access can add Area events.');
  const areaId = requireArea(profile);
...
async function updateEvent(req, res) {
  const { supabase, profile } = await requireAuthenticatedProfile(req);
  requireAreaAdmin(profile, 'Only Area-level servant access can edit Area events.');
  const areaId = requireArea(profile);
...
async function deleteEvent(req, res) {
  const { supabase, profile } = await requireAuthenticatedProfile(req);
  requireAreaAdmin(profile, 'Only Area-level servant access can delete Area events.');
  const areaId = requireArea(profile);
