...
- `POST /api/admin/members/:id/change-email` (Area-level administrators only)
...
- Area-level administrator roles are Couple Coordinator/s, Area Servant, Area LIT Servant, Campus Servant, and Area Kids Servant. The database does not define a separate `super_admin` role.
