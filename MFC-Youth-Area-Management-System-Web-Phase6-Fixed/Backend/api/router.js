...
export default async function handler(req, res) {
  try {
    ...
    const adminEmailMatch = /^admin\/members\/([^/]+)\/change-email$/.exec(route);
    if (adminEmailMatch) {
      req.query = { ...(req.query || {}), id: decodeURIComponent(adminEmailMatch[1]) };
      return await adminChangeEmail(req, res);
    }
