...
if (logoutBtn) {
  ...
  if (session?.role !== 'member') {
    if (session?.backendAuth && !session?.demo) {
      ...
      changeEmailButton.className = 'sidebar-account-action';
      changeEmailButton.textContent = 'Change Email';
      changeEmailButton.onclick = () => navigateWithLoader('/change-email');
      logoutBtn.parentElement?.insertBefore(changeEmailButton, logoutBtn);
    }
...
function memberModal(id = null) {
  if (id) {
    ...
    toast('Only Area-level servant access and Chapter Servants can add members.', 'error');
    return;
  }
        ...
        ${isChapterServantSession() && !id
          ? `
            ...
            <small class="field-help">
              Chapter Servants can create member accounts, but only Area-level servant access can grant elevated system access.
            </small>
          `
          : `
            <select
...
window.eventModal = function (
  ...
  if (denyUnlessAreaAdmin('Only Area-level servant access can create or edit Area events.')) return;
...
window.deleteEvent = async id => {
  if (denyUnlessAreaAdmin('Only Area-level servant access can delete Area events.')) return;
...
window.participantModal = (
  ...
  if (denyUnlessAreaAdmin('Only Area-level servant access can manage event participants.')) return;
