...
async function apiJson(path, options = {}) {
  ...
  const accessToken = activeSession?.backendAuth ? String(activeSession.accessToken || '') : '';
...
if (loginForm) {
  loginForm.addEventListener('submit', async event => {
    ...
    // Production authentication is backend-mediated. Browser-only prototype
    // accounts are intentionally not accepted as an authentication fallback;
    // otherwise a backend denial (inactive account, revoked session, etc.) could
    // be bypassed with stale localStorage data. The dedicated demo account above
    // remains the only local presentation-mode exception.
    try {
...
if (backToLoginButton) {
  backToLoginButton.addEventListener('click', async () => {
    ...
    try {
      if (activeSession?.backendAuth && !activeSession?.demo) {
      ...
      console.warn('Server session revocation could not be confirmed:', error?.message || error);
    } finally {
      clearSession();
...
if (changeEmailForm) {
  ...
  const currentEmail = document.getElementById('changeEmailCurrent');
  const backButton = document.getElementById('backFromEmailButton');

  if (!session) {
    navigateWithLoader('/', true);
  } else if (!session.backendAuth || session.demo) {
    showMessage('changeEmailMessage', 'Email changes are available only for cloud-backed accounts.');
    changeEmailForm.querySelectorAll('input, button[type="submit"]').forEach(el => { el.disabled = true; });
  } else {
    if (currentEmail) currentEmail.textContent = session.email || '—';

    // Refresh the authoritative Auth email on entry. This also gives the
    // backend a trusted opportunity to repair members.email after a previously
    // confirmed Secure Email Change.
    apiJson('/api/auth/me', { method: 'GET' })
      .then(payload => {
        const freshEmail = normalizeEmail(payload?.user?.email || '');
        ...
        updateSession(session);
        if (currentEmail) currentEmail.textContent = freshEmail;
      })
      .catch(() => {
        // The form can still operate with the currently validated session;
        // the backend will re-validate it on submission.
      });

    changeEmailForm.addEventListener('submit', async event => {
      event.preventDefault();
      const submit = changeEmailForm.querySelector('[type="submit"]');
      const newEmail = normalizeEmail(document.getElementById('newAccountEmail')?.value || '');

      if (!isValidEmail(newEmail)) {
        showMessage('changeEmailMessage', 'Enter a valid email address.');
        return;
      }
      if (newEmail === normalizeEmail(session.email || '')) {
        showMessage('changeEmailMessage', 'That email address is already linked to your account.');
        return;
      }

      setButtonBusy(submit, true, 'Sending Confirmation…');
      try {
        const payload = await apiJson('/api/auth/change-email', {
          method: 'POST',
          body: JSON.stringify({ newEmail })
        });
        showMessage('changeEmailMessage', payload.message || 'Email change requested. Check your inboxes to confirm the change.', 'success');
        changeEmailForm.reset();
      } catch (error) {
        showMessage('changeEmailMessage', error?.message || 'Unable to start the email change.');
      } finally {
        setButtonBusy(submit, false);
      }
    });
  ...
  backButton?.addEventListener('click', () => navigateWithLoader(destinationFor(session || {})));
}
