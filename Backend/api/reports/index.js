...
async function saveReport(req, res, isUpdate) {
  ...
  if (!isSuperAdminRole(profile.role) && !isChapterServantRole(profile.role)) return sendJson(res, 403, { ok: false, error: 'You do not have permission to manage activity reports.' });
  const areaId = requireArea(profile);
...
async function deleteReport(req, res) {
  const { supabase, profile } = await requireAuthenticatedProfile(req);
  if (!isSuperAdminRole(profile.role) && !isChapterServantRole(profile.role)) return sendJson(res, 403, { ok: false, error: 'You do not have permission to delete activity reports.' });
  const areaId = requireArea(profile);
